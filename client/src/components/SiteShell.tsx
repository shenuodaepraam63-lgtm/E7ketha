...
export function Breadcrumbs({ items }: { items: string[] }) {
  return <div className="mb-8 flex flex-wrap items-center gap-3 text-xs text-muted-foreground"><button type="button" onClick={() => window.history.back()} className="rounded-lg border border-border px-2 py-1 font-bold hover:text-[#675de8]">رجوع</button><Link href="/" className="hover:text-[#675de8]">الرئيسية</Link>{items.map((item) => <span key={item} className="flex items-center gap-2"><span className="text-border">/</span><span className="text-foreground/75">{item}</span></span>)}</div>;
}
...
export function EmptyState({ title, description, action, href = '/explore' }: { title: string; description: string; action?: string; href?: string }) {
  return <div className="flex min-h-[280px] flex-col items-center justify-center rounded-[24px] border border-dashed border-border bg-card/60 px-6 text-center"><div className="mb-5 grid size-14 place-items-center rounded-[18px] bg-[#f0eeff] text-xl text-[#6e64e9] dark:bg-[#24224c]">✦</div><h2 className="text-lg font-extrabold">{title}</h2><p className="mt-2 max-w-sm text-sm leading-7 text-muted-foreground">{description}</p>{action && <Link href={href} className="mt-5 rounded-xl bg-[#171e42] px-5 py-3 text-xs font-bold text-white dark:bg-[#eeeefe] dark:text-[#101936]">{action}</Link>}</div>;
}
