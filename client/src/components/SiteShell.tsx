...
export function SiteShell({ children, theme, onThemeToggle, footer = true }: { children: ReactNode; theme: 'light' | 'dark'; onThemeToggle: () => void; footer?: boolean }) {
  ...
  return <div dir="rtl" className="min-h-screen overflow-x-hidden"><SiteHeader theme={theme} onThemeToggle={onThemeToggle} /><main key={location} className="page-transition">{children}</main>{footer && <SiteFooter />}<MobileBottomNav /></div>;
}
