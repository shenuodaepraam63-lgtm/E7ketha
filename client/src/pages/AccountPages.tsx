  ...
  return (
    <div className="relative flex min-h-[calc(100vh-72px)] items-center justify-center px-4 py-10 sm:py-12">
      ...
      <style>{`
        ...
        .auth-input-focus {
          box-shadow: 0 0 0 3px rgba(103, 93, 232, 0.28), 0 0 24px rgba(103, 93, 232, 0.18);
        }
        .auth-btn-shine {
        ...
        .auth-card-shell {
          position: relative;
          height: 100%;
          background: linear-gradient(165deg, rgba(24, 30, 62, 0.98) 0%, rgba(14, 18, 42, 0.98) 100%);
      ...
      >
        ...
        <div
          ...
          style={{
            transform: flipping ? undefined : `rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`,
          }}
        >
...
export function PasswordResetPage() {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if ((event === 'SIGNED_IN' || event === 'INITIAL_SESSION') && session) markReady();
    });
    ...
    (async () => {
      try {
        ...
        if (hash) {
          ...
          if (access_token && refresh_token && (type === 'recovery' || type === 'magiclink' || !type)) {
        ...
        markFail('افتح رابط الاستعادة من بريدك الإلكتروني. الرابط صالح لفترة محدودة.');
      } catch (e) {
    ...
    <div className="relative flex min-h-[calc(100vh-72px)] items-center justify-center px-4 py-10">
      ...
      <div className="relative z-10 w-full max-w-md rounded-[26px] border border-white/10 bg-[#11183a] p-6 shadow-2xl sm:p-8">
        <div className="mb-6 text-center">
          ...
          <p className="mt-2 text-sm leading-7 text-white/50">أدخل كلمة المرور الجديدة ثم أكّدها للتحديث.</p>
        </div>
        ...
        {checking ? (
          ...
          <div className="space-y-4 text-center">
            <p className="text-sm leading-7 text-white/55">
              {errorMsg || 'افتح رابط الاستعادة من بريدك الإلكتروني.'}
            </p>
            <Link
              href="/login"
              className="inline-flex rounded-xl bg-[#675de8] px-5 py-3 text-xs font-bold text-white transition hover:brightness-110"
            >
              العودة لتسجيل الدخول
            </Link>
          </div>
      ...
      <div className="my-8 flex gap-2 border-b border-border pb-3">
        {(
          ...
          <button
            ...
            className={tab === value ? 'px-4 pb-2 text-xs font-bold border-b-2 border-[#7067ef] text-[#675de8]' : 'px-4 pb-2 text-xs font-bold text-muted-foreground'}
          >
            {label}
          </button>
        ))}
    ...
    <div className="container py-10 md:py-16">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-[18px] border border-border bg-card p-5">
          ...
          <Link href="/saved-quotes" className="mt-1 block text-xs font-bold text-[#675de8]">اقتباس محفوظ</Link>
        </div>
        <div className="rounded-[18px] border border-border bg-card p-5">
  ...
  return (
    <div className="container py-10 md:py-16">
      ...
      {query.isLoading ? (
        ...
        <div className="mt-10">
          <EmptyState title="لم تحفظ اقتباسات بعد" description="اضغط زر حفظ الاقتباس في أي صفحة اقتباس." action="تصفح الاقتباسات" href="/quotes" />
        </div>
      )}
    </div>
  );
