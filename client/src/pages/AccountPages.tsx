...
export function ProfilePage() { const { user, loading, logout } = useAuth(); const list = trpc.readingList.list.useQuery(undefined, { enabled: Boolean(user) }); const quoteList = trpc.quotes.saved.useQuery(undefined, { enabled: Boolean(user) }); const stats = { saved: list.data?.length ?? 0, finished: list.data?.filter((item) => item.status === 'finished').length ?? 0 }; if (loading) return <div className="container py-20 text-center">جارٍ تحميل الحساب...</div>; if (!user) return <div className="container py-16"><EmptyState title="سجّل الدخول إلى حسابك" description="أنشئ حسابًا للوصول إلى قائمتك وتقييماتك." action="تسجيل الدخول" href="/login" /></div>; return <div className="container py-10 md:py-16"><Breadcrumbs items={['حسابي']} /><section className="flex flex-col gap-5 rounded-[24px] border border-border bg-card p-6 sm:flex-row sm:items-center"><div className="grid size-20 place-items-center rounded-[24px] bg-[#eceaff] text-2xl font-extrabold text-[#675de8]">{(user.name ?? 'ر')[0]}</div><div><h1 className="text-2xl font-extrabold">مرحبًا، {user.name ?? 'قارئ رِواية'}</h1><p className="mt-1 text-xs text-muted-foreground">{user.email}</p></div><button onClick={() => void logout()} className="sm:mr-auto rounded-xl border border-border px-4 py-3 text-xs font-bold">تسجيل الخروج</button></section><div className="mt-8 grid gap-4 sm:grid-cols-3"><div className="rounded-[18px] border border-border bg-card p-5"><Heart size={18} className="mb-4 text-[#7067ef]" /><strong className="block text-2xl font-extrabold">{stats.saved}</strong><span className="mt-1 block text-xs text-muted-foreground">رواية محفوظة</span></div><div className="rounded-[18px] border border-border bg-card p-5"><Heart size={18} className="mb-4 text-[#675de8]" fill="currentColor" /><strong className="block text-2xl font-extrabold">{quoteList.data?.length ?? 0}</strong><Link href="/saved-quotes" className="mt-1 block text-xs font-bold text-[#675de8]">اقتباس محفوظ</Link></div><div className="rounded-[18px] border border-border bg-card p-5"><Check size={18} className="mb-4 text-emerald-500" /><strong className="block text-2xl font-extrabold">{stats.finished}</strong><span className="mt-1 block text-xs text-muted-foreground">روايات تمت قراءتها</span></div></div><section className="mt-12"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-extrabold">رواياتك المحفوظة</h2><Link href="/my-list" className="text-xs font-bold text-[#675de8]">عرض القائمة</Link></div>{list.data?.length ? <div className="grid grid-cols-2 gap-5 sm:grid-cols-4">{list.data.slice(0, 4).map((item) => <NovelCard key={item.id} novel={toNovel({ id: item.novelId, slug: item.slug, title: item.title, coverUrl: item.coverUrl, description: item.description, rating: item.rating, parts: item.parts, status: item.novelStatus, author: item.author, authorSlug: item.authorSlug })} />)}</div> : <EmptyState title="ابدأ بناء قائمتك" description="لم تحفظ روايات بعد." action="استكشف الروايات" />}</section><section className="mt-12"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-extrabold">اقتباساتك المحفوظة</h2><Link href="/saved-quotes" className="text-xs font-bold text-[#675de8]">عرض الكل</Link></div>{quoteList.data?.length ? <div className="grid gap-4 md:grid-cols-2">{quoteList.data.slice(0, 2).map((item) => <Link key={item.id} href={`/quotes/${item.id}`} className="rounded-2xl border border-border bg-card p-5 text-sm font-bold leading-8">“{item.quote_text}”</Link>)}</div> : <EmptyState title="احفظ أول اقتباس" description="اضغط زر الحفظ في صفحة أي اقتباس." action="تصفح الاقتباسات" href="/quotes" />}</section></div>; }
...
export function SavedQuotesPage() {
  ...
  if (!user) return <div className="container py-16"><EmptyState title="سجّل الدخول لمحفوظاتك" description="احفظ الاقتباسات التي تلمس قلبك وارجع إليها من أي جهاز." action="تسجيل الدخول" href="/login" /></div>;
  return <div className="container py-10 md:py-16"><Breadcrumbs items={['حسابي', 'اقتباساتي المحفوظة']} /><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold text-[#675de8]">مكتبتك الشخصية</p><h1 className="mt-2 text-4xl font-extrabold">اقتباساتي المحفوظة</h1><p className="mt-3 text-sm text-muted-foreground">مجموعة الاقتباسات التي اخترت الاحتفاظ بها لتعود إليها عندما تحتاجها.</p></div><Link href="/quotes" className="inline-flex rounded-xl bg-[#171e42] px-4 py-3 text-xs font-extrabold text-white">اكتشف اقتباسات جديدة</Link></div>{query.isLoading ? <p className="mt-10 text-sm text-muted-foreground">جارٍ تحميل محفوظاتك...</p> : query.data?.length ? <div className="mt-10 grid gap-5 md:grid-cols-2">{query.data.map((item) => <article key={item.id} className="rounded-[24px] border border-border bg-card p-6 shadow-sm"><Heart className="mb-5 text-[#675de8]" size={19} fill="currentColor" /><blockquote className="text-lg font-extrabold leading-9">“{item.quote_text}”</blockquote><div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4 text-xs"><span className="text-muted-foreground">{item.author_name || item.speaker || 'القائل غير محدد'}{item.book_title ? ` · ${item.book_title}` : ''}</span><Link href={`/quotes/${item.id}`} className="font-extrabold text-[#675de8]">فتح الاقتباس</Link></div></article>)}</div> : <div className="mt-10"><EmptyState title="لم تحفظ اقتباسات بعد" description="اضغط زر حفظ الاقتباس في أي صفحة اقتباس لبناء مجموعتك الخاصة." action="تصفح الاقتباسات" href="/quotes" /></div>}</div>;
}
