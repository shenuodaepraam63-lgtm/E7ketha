import { BookOpen, Quote, Loader2, Search, Share2 } from 'lucide-react';
...
export default function QuotesPage() {
  ...
  return <div className="container py-10 md:py-16">
    ...
    <PageIntro eyebrow="بين السطور" title="اقتباسات الكتب" description="اكتشف اقتباسات عن الحياة والحب والنجاح والفلسفة من كتّاب وروايات مختلفة." /><div className="mb-8 flex items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3"><Search size={18} className="text-[#675de8]" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="ابحث عن اقتباس أو كاتب أو كتاب..." /></div>
