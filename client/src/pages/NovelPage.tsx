...
import { AdSlot, AutoRelaxedAd } from '@/components/AdSlot';
...
export default function NovelPage() {
  ...
  useEffect(() => {
    ...
    const preferred = query.data.slug;
    if (/^\d+$/.test(current) && preferred && current !== preferred) {
...
function NovelDetails({ novel }: { novel: ReturnType<typeof toNovel> }) {
  ...
  const share = () => {
    const shareUrl = `${window.location.origin}/books/${novel.slug}`;
