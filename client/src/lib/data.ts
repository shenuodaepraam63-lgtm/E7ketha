export type Novel = { id: string; slug: string; title: string; author: string; authorSlug: string; cover: string; genres: string[]; rating: number; parts: number; status: string; description: string; accent: string; publicationYear?: number | null; language?: string | null };
...
export function toNovel(row: { id: number; slug: string; title: string; coverUrl: string | null; description: string | null; rating: number; parts: number; status: string; author: string; authorSlug: string; publicationYear?: number | null; language?: string | null }): Novel {
  return { id: String(row.id), slug: row.slug, title: row.title, author: row.author, authorSlug: row.authorSlug, cover: row.coverUrl ?? coverFallback, genres: [], rating: row.rating / 100, parts: row.parts, status: statusLabels[row.status] ?? row.status, description: row.description ?? '', accent: '#7067ef', publicationYear: row.publicationYear, language: row.language };
}
