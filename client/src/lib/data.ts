...
export type Novel = { id: string; slug: string; title: string; author: string; authorSlug: string; cover: string; genres: string[]; rating: number; parts: number; status: string; description: string; rightsNote?: string | null; accent: string; publicationYear?: number | null; language?: string | null; links?: NovelLink[] };
...
export const navItems = [{ label: 'الرئيسية', href: '/' }, { label: 'استكشف', href: '/explore' }, { label: 'الموضوعات', href: '/topics' }, { label: 'مقالات', href: '/articles' }, { label: 'التصنيفات', href: '/genres/fantasy' }, { label: 'المؤلفون', href: '/authors/amr-abdel-hamid' }, { label: 'السلاسل', href: '/series/zikola' }];
...
export function toNovel(row: { id: number; slug: string; title: string; coverUrl: string | null; description: string | null; rating: number; parts: number; status: string; author: string; authorSlug: string; publicationYear?: number | null; language?: string | null; rightsNote?: string | null; links?: NovelLink[] }): Novel {
  return { id: String(row.id), slug: row.slug, title: row.title, author: row.author, authorSlug: row.authorSlug, cover: optimizeCoverUrl(row.coverUrl ?? coverFallback, 400), genres: [], rating: row.rating / 100, parts: row.parts, status: statusLabels[row.status] ?? row.status, description: row.description ?? '', rightsNote: row.rightsNote, accent: '#7067ef', publicationYear: row.publicationYear, language: row.language, links: row.links ?? [] };
}
