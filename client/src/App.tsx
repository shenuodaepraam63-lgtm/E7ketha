...
function SeoManager({ location }: { location: string }) {
  useEffect(() => {
    ...
    const titles: Record<string, string> = {
      ...
      '/privacy': `سياست الخصوصية | ${SITE_NAME}`,
    ...
    const title = titles[pathname] ?? (isPrivate ? `حسابي | ${SITE_NAME}` : HOME_TITLE);
    const description = isPrivate
      ? `هذه الصفحة مخصصة للمستخدمين المسجلين في ${SITE_NAME}.`
      : pathname === '/quotes'
        ? 'اقرأ واقتبس وشارك أجمل الاقتباسات العربية عن الحب والحياة والفلسفة والقراءة من الروايات والكتّاب.'
        : pathname === '/articles'
          ? 'مقالات أدبية وترشيحات قراءة من منصة اكتشاف الروايات العربية.'
          : DEFAULT_DESCRIPTION;
    ...
    // Do not overwrite SSR unique titles/descriptions on entity pages
    if (!isEntityPage) {
      ...
      <Suspense fallback={<LoadingPage />}>
        {isAuthPage ? (
          ...
          <Switch>
            ...
            <Route path="/quotes" nt={QuoteCategoryPage} />
