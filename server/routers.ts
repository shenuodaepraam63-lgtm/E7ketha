...
export const appRouter = router({
  ...
  adminQuotes: router({
    ...
    bulkCreate: adminProcedure.input(z.object({ sourceUrl: z.string().url().max(2000), author: z.string().max(255).optional(), book: z.string().max(255).optional(), instructions: z.string().max(2000).optional(), quotes: z.array(z.object({ quote_text: z.string().min(3).max(2000), speaker: z.string().max(255).nullable().optional(), book_title: z.string().max(255).nullable().optional(), category: z.string().max(80).nullable().optional(), status: z.enum(['draft', 'published']) })).min(1).max(500) })).mutation(async ({ input }) => { const [existingRows, authors, books] = await Promise.all([existingQuoteTexts(), listQuoteAuthors(), listQuoteBooks()]); const existing = new Set(existingRows.map((item) => item.quote_text.trim())); const fresh = input.quotes.filter((quote) => !existing.has(quote.quote_text.trim())); const matchedAuthor = matchEntity(input.author, authors); const matchedBook = matchEntity(input.book, books); const importRow = await createQuoteImport({ source_url: input.sourceUrl, author: input.author, book: input.book, instructions: input.instructions, quote_count: fresh.length }); for (let index = 0; index < fresh.length; index += 1) { const quote = fresh[index]; const author = matchEntity(quote.speaker || input.author, authors) ?? matchedAuthor; const book = matchEntity(quote.book_title || input.book, books) ?? matchedBook; await createQuote({ ...quote, speaker: quote.speaker ?? null, book_title: quote.book_title ?? null, author_id: author?.id ?? null, novel_id: book?.id ?? null, category: quote.category ?? null, source_url: input.sourceUrl, import_id: importRow.id, position: index + 1 }); } return { count: fresh.length, skippedDuplicates: input.quotes.length - fresh.length, importId: importRow.id, authorMatched: Boolean(matchedAuthor), bookMatched: Boolean(matchedBook), authorSlug: matchedAuthor?.slug ?? null, bookSlug: matchedBook?.slug ?? null }; }),
  }),
});
