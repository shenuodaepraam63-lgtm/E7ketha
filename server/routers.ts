...
import { createQuote, createQuoteImport, deleteQuote, existingQuoteTexts, getQuote, improveQuote, listQuoteImports, listQuotes, previewQuotesFromUrl, updateQuote } from './quotes';
...
export const appRouter = router({
  ...
  adminQuotes: router({
    ...
    previewImport: adminProcedure.input(z.object({ url: z.string().url().max(2000), author: z.string().max(255).optional(), book: z.string().max(255).optional(), instructions: z.string().max(2000).optional(), useAi: z.boolean().default(true) })).mutation(({ input }) => previewQuotesFromUrl(input)),
    imports: adminProcedure.query(() => listQuoteImports()),
    bulkCreate: adminProcedure.input(z.object({ sourceUrl: z.string().url().max(2000), author: z.string().max(255).optional(), book: z.string().max(255).optional(), instructions: z.string().max(2000).optional(), quotes: z.array(z.object({ quote_text: z.string().min(3).max(2000), speaker: z.string().max(255).nullable().optional(), book_title: z.string().max(255).nullable().optional(), category: z.string().max(80).nullable().optional(), status: z.enum(['draft', 'published']) })).min(1).max(500) })).mutation(async ({ input }) => { const existing = new Set((await existingQuoteTexts()).map((item) => item.quote_text.trim())); const fresh = input.quotes.filter((quote) => !existing.has(quote.quote_text.trim())); const importRow = await createQuoteImport({ source_url: input.sourceUrl, author: input.author, book: input.book, instructions: input.instructions, quote_count: fresh.length }); for (let index = 0; index < fresh.length; index += 1) { const quote = fresh[index]; await createQuote({ ...quote, speaker: quote.speaker ?? null, book_title: quote.book_title ?? null, novel_id: null, category: quote.category ?? null, source_url: input.sourceUrl, import_id: importRow.id, position: index + 1 }); } return { count: fresh.length, skippedDuplicates: input.quotes.length - fresh.length, importId: importRow.id }; }),
  }),
});
