...
function decodeHtml(value: string) { return value.replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').replace(/&quot;/gi, '"').replace(/&#39;|&apos;/gi, "'").replace(/&lt;/gi, '<').replace(/&gt;/gi, '>').replace(/&#x([0-9a-f]+);/gi, (_, hex) => String.fromCodePoint(parseInt(hex, 16))).replace(/&#(\d+);/g, (_, dec) => String.fromCodePoint(Number(dec))); }
...
export async function previewQuotesFromUrl(input: { url: string; author?: string; book?: string; instructions?: string; useAi?: boolean }): Promise<QuoteImportPreview> {
  ...
  let resolvedAuthor = author; let resolvedBook = book; if (input.useAi) { try { const ai = await extractWithAi(cleanText(html.replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>/gi, ' ')), input.instructions ?? 'استخرج الاقتباسات فقط'); extracted = ai.quotes; resolvedAuthor ||= ai.author; resolvedBook ||= ai.book; } catch (error) { console.warn('[Quotes] AI extraction unavailable; using deterministic extraction', error); } }
  const unique = Array.from(new Set(extracted.map(cleanImportedQuote).filter((quote) => quote.length >= 12 && !/^tags\s*:/i.test(quote)))).slice(0, 500); if (!unique.length) throw new Error('لم أجد اقتباسات واضحة. جرّب إضافة تعليمات مثل: كل اقتباس داخل blockquote أو كل سطر اقتباس مستقل.');
  return { sourceUrl: input.url, author: resolvedAuthor, book: resolvedBook, quotes: unique.map((quote_text) => ({ quote_text, speaker: resolvedAuthor, book_title: resolvedBook, category: '', status: 'draft' as const })) };
}
