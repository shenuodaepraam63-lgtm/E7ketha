...
export async function previewQuotesFromUrl(input: { url: string; author?: string; book?: string; instructions?: string; useAi?: boolean }): Promise<QuoteImportPreview> {
  ...
  let resolvedAuthor = author; let resolvedBook = book; if (input.useAi) { const ai = await extractWithAi(cleanText(html.replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>/gi, ' ')), input.instructions ?? 'استخرج الاقتباسات فقط'); extracted = ai.quotes; resolvedAuthor ||= ai.author; resolvedBook ||= ai.book; }
