...
async function getNovelBySlugFromRest(slug: string) {
  ...
  const links = await supabaseRest<any[]>('novelLinks', `select=id,label,url,type,displayOrder&novelId=eq.${row.id}&order=displayOrder.asc,id.asc`);
  return { id: row.id, slug: normalizeNovelSlug(row.slug, row.title), title: row.title, coverUrl: row.coverUrl, description: row.description, rating: row.rating, ratingCount: row.ratingCount, parts: row.parts, status: row.status, publicationYear: row.publicationYear, language: row.language, author: author?.name ?? 'مؤلف غير معروف', authorSlug: author?.slug ?? '', authorId: row.authorId, links };
}
...
async function searchNovelsFromRest(filters: NovelSearchFilters = {}) {
  ...
  if (minRating) rows = rows.filter((row) => Number(row.rating ?? 0) >= Math.round(minRating * 100));
...
export async function getNovelBySlug(slug: string) {
  ...
  if (result[0]) {
    const links = await db.select({ id: novelLinks.id, label: novelLinks.label, url: novelLinks.url, type: novelLinks.type, displayOrder: novelLinks.displayOrder }).from(novelLinks).where(eq(novelLinks.novelId, result[0].id)).orderBy(asc(novelLinks.displayOrder), asc(novelLinks.id));
    return { ...result[0], slug: normalizeNovelSlug(result[0].slug, result[0].title), links };
  }
...
export type NovelLinkInput = { label: string; url: string; type: 'read' | 'download'; displayOrder?: number };

export async function replaceNovelLinks(novelId: number, links: NovelLinkInput[]) {
  const db = await requireDb();
  await db.delete(novelLinks).where(eq(novelLinks.novelId, novelId));
  if (links.length) {
    await db.insert(novelLinks).values(links.map((link, index) => ({ novelId, label: link.label, url: link.url, type: link.type, displayOrder: link.displayOrder ?? index })));
  }
  return db.select({ id: novelLinks.id, label: novelLinks.label, url: novelLinks.url, type: novelLinks.type, displayOrder: novelLinks.displayOrder }).from(novelLinks).where(eq(novelLinks.novelId, novelId)).orderBy(asc(novelLinks.displayOrder), asc(novelLinks.id));
}
...
export async function listAdminNovels() {
  ...
  return Promise.all(rows.map(async (row) => ({
    ...row,
    links: await db.select({ id: novelLinks.id, label: novelLinks.label, url: novelLinks.url, type: novelLinks.type, displayOrder: novelLinks.displayOrder }).from(novelLinks).where(eq(novelLinks.novelId, row.id)).orderBy(asc(novelLinks.displayOrder), asc(novelLinks.id)),
  })));
}
...
export async function createNovel(input: AdminNovelInput) {
  ...
  if (links.length) await db.insert(novelLinks).values(links.map((link, index) => ({ novelId: row.id, label: link.label, url: link.url, type: link.type, displayOrder: link.displayOrder ?? index })));
  return row;
}
...
export async function deleteNovel(id: number) {
  const db = await requireDb();
  await db.delete(novelGenres).where(eq(novelGenres.novelId, id));
  await db.delete(novelLinks).where(eq(novelLinks.novelId, id));
