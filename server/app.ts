...
async function renderPublicSeo(pathname: string) {
  ...
  if (/^\/(?:books|novel|novels)\/[^/]+$/.test(normalized)) {
    ...
    return renderSeoDocument(readClientTemplate(), {
      ...
      jsonLd: { '@context': 'https://schema.org', '@type': 'Book', '@id': canonical, name: novel.title, description, image: novel.coverUrl || undefined, inLanguage: novel.language || 'ar', author: { '@type': 'Person', name: novel.author, url: authorUrl }, mainEntityOfPage: { '@type': 'WebPage', '@id': canonical, url: canonical, name: novel.title }, ...(Number(novel.ratingCount) > 0 && Number(novel.rating) > 0 ? { aggregateRating: { '@type': 'AggregateRating', ratingValue: (Number(novel.rating) / 100).toFixed(1), bestRating: '5', worstRating: '1', ratingCount: Number(novel.ratingCount) } } : {}), url: canonical },
  ...
  if (/^\/quotes\/\d+$/.test(normalized)) {
    ...
    return renderSeoDocument(readClientTemplate(), { title: `اقتباس من ${quote.book_title || 'رواية'} | رِواية`, description, canonical, jsonLd: { '@context': 'https://schema.org', '@type': 'Quotation', '@id': canonical, name: `اقتباس من ${quote.book_title || 'رواية'}`, text: quote.quote_text, inLanguage: 'ar', author: quote.author_name ? { '@type': 'Person', name: quote.author_name, ...(authorUrl ? { url: authorUrl } : {}) } : undefined, isPartOf: quote.book_title ? { '@type': 'Book', name: quote.book_title, ...(bookUrl ? { '@id': bookUrl, url: bookUrl } : {}) } : undefined, mainEntityOfPage: { '@type': 'WebPage', '@id': canonical, url: canonical }, url: canonical }, content: `<main lang="ar" dir="rtl"><nav><a href="${origin}/">الرئيسية</a> / <a href="${origin}/quotes">الاقتباسات</a></nav><article><h1>اقتباس من ${htmlEscape(quote.book_title || 'رواية')}</h1><blockquote>${htmlEscape(quote.quote_text)}</blockquote>${quote.author_name ? `<p>— <a href="${origin}/authors/${htmlEscape(quote.author_slug || '')}">${htmlEscape(quote.author_name)}</a></p>` : ''}</article></main>` });
  }
